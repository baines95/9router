...
function ModelRow({ model, fullModel, alias, selected, onToggleSelect, copied, onCopy }) {
 return (
 <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-border/50 hover:bg-sidebar/50">
 <button
 type="button"
...
function PassthroughModelsSection({ providerAlias, modelAliases, copied, onCopy, onSetAlias, onDeleteAlias }) {
 ...
 className="w-full px-3 py-2 text-sm border border-border/50 rounded-lg bg-background focus:outline-none focus:border-primary"
 />
 </div>
...
function PassthroughModelRow({ modelId, fullModel, copied, onCopy, onDeleteAlias }) {
 return (
 <div className="flex items-center gap-3 p-3 rounded-lg border border-border/50 hover:bg-sidebar/50">
 <span className="material-symbols-outlined text-base text-text-muted">smart_toy</span>
...
function ConnectionRow({ connection, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onEdit, onDelete }) {
 ...
 <span className="text-xs text-destructive truncate max-w-[300px]"title={connection.lastError}>
 ...
 <button onClick={onDelete} className="p-2 hover:bg-destructive/10 rounded text-destructive">
