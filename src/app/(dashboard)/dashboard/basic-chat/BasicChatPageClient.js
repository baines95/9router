...
export default function BasicChatPageClient({ initialData }) {
 ...
 {/* Top Header */}
 <header className="h-14 border-b border-border/50 flex items-center justify-between px-4 bg-muted/20 shrink-0">
 <div className="flex items-center gap-2 min-w-0">
 <DropdownMenu>
 <DropdownMenuTrigger asChild>
 <Button variant="ghost"size="sm"className="h-9 px-3 gap-2 font-medium rounded-lg hover:bg-muted">
 <Cpu className="size-4 text-primary"/>
 <span className="truncate max-w-[200px]">{activeModel?.name ||"Select Model"}</span>
 <ChevronDown className="size-3.5 opacity-50"/>
 </Button>
 </DropdownMenuTrigger>
 <DropdownMenuContent align="start"className="w-[320px] max-h-[60vh] p-0 overflow-hidden rounded-xl border-border/50">
 <ScrollArea className="h-full">
 {providerGroups.map(group => (
 <div key={group.providerId} className="p-1">
 <DropdownMenuLabel className="text-xs font-semibold uppercase tracking-tight text-muted-foreground/60 px-2 py-1.5">{group.providerName}</DropdownMenuLabel>
 {group.models.map(m => (
 ...
 <p className="text-xs text-muted-foreground truncate opacity-70">{m.requestModel}</p>
 </div>
 ...
 <DropdownMenuLabel className="text-xs font-semibold uppercase tracking-tight px-3">Recent Conversations</DropdownMenuLabel>
 <DropdownMenuSeparator />
 <ScrollArea className="h-[400px]">
 {sessionItems.length === 0 ? (
 <div className="p-8 text-center opacity-30 text-xs font-medium uppercase tracking-tight">No history</div>
 ) : sessionItems.map(s => (
 ...
 <span className="text-xs font-medium truncate w-full">{s.title}</span>
 <span className="text-xs text-muted-foreground">{formatRelativeTime(s.updatedAt)} ago</span>
 </DropdownMenuItem>
 ))}
 </ScrollArea>
 </DropdownMenuContent>
 ...
 <h2 className="text-lg font-medium">Ready to chat?</h2>
 ...
 <span className="text-xs font-semibold uppercase tracking-tight opacity-50">{isUser ?"USER": activeModel?.name ||"AI"}</span>
 </div>
 <div className={cn(
"max-w-[85%] rounded-2xl p-4 text-[14px] leading-relaxed",
 isUser ?"bg-primary text-primary-foreground font-medium":"bg-muted/40 border border-border/50"
 )}>
 ...
 {attachments.map(a => (
 <div key={a.id} className="h-7 px-2 rounded-md bg-muted border border-border/50 flex items-center gap-2 text-xs font-medium">
 ...
 <textarea
 className="w-full bg-muted/50 border border-border/50 rounded-2xl px-4 py-4 pr-12 min-h-[60px] max-h-[200px] text-sm focus:ring-1 focus:ring-primary/20 outline-none resize-none transition-all"
 ...
 <Button 
 size="icon"
 className={cn("size-8 rounded-full", canSend ?"bg-primary":"bg-muted opacity-30")} 
 ...
 <p className="text-center text-xs font-medium uppercase tracking-tight text-muted-foreground py-3 opacity-40">System Mesh Infrastructure • Real-time Traffic</p>
 </div>
 </footer>
 </div>
 );
