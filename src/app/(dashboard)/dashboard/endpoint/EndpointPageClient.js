...
"messages": [{"role":"user","content":"Hello!"}]
 ...
 <Badge variant="outline"className="bg-primary/10 text-primary border-primary/20 font-medium h-5 px-2">
 <Activity className="size-3 mr-1.5"/> GATEWAY ACTIVE
 </Badge>
 {tunnelEnabled && <Badge variant="outline"className="text-muted-foreground border-orange-500/20 bg-muted/30 h-5 px-2">CLOUDFLARE</Badge>}
 {tsEnabled && <Badge variant="outline"className="text-primary border-purple-500/20 bg-primary/10 h-5 px-2">TAILSCALE</Badge>}
 </div>
 <CardTitle className="text-2xl font-medium tracking-tight">Public API Infrastructure</CardTitle>
 ...
 <Button variant="ghost"size="icon"className="size-6"onClick={() => copy(key.key, key.id)}>{copied === key.id ? <Check className="size-3 text-primary"/> : <Copy className="size-3"/>}</Button>
 </div>
 </div>
 </div>
 <div className="flex items-center gap-4 pl-4 border-l">
 <Switch checked={key.isActive ?? true} onCheckedChange={(v) => handleToggleKey(key.id, v)} className="scale-75"/>
 <Button variant="ghost"size="icon"className="size-8 text-muted-foreground hover:text-destructive"onClick={() => handleDeleteKey(key.id)}><Trash2 className="size-3.5"/></Button>
 </div>
 </div>
 ))}
 </div>
 ...
 <CardTitle className="text-xs font-semibold uppercase tracking-tight text-muted-foreground/60 flex items-center gap-2"><Terminal className="size-3.5"/> Integration</CardTitle>
 </CardHeader>
 ...
 <Button variant="outline"size="sm"className="w-full h-8 text-xs font-medium uppercase tracking-tight"onClick={() => copy(testCurl,"curl")}><Copy className="size-3 mr-2"/> Copy Script</Button>
 </CardContent>
 </Card>

 <Card className="border-border/50 p-0 overflow-hidden">
 <CardHeader className="p-4 border-b bg-muted/20">
 <CardTitle className="text-xs font-semibold uppercase tracking-tight text-muted-foreground/60 flex items-center gap-2"><Settings2 className="size-3.5"/> Advanced</CardTitle>
 </CardHeader>
 <CardContent className="p-4 space-y-4">
 ...
 <DialogHeader><DialogTitle className="text-primary flex items-center gap-2"><CheckCircle2 className="size-5"/> Token Generated</DialogTitle><DialogDescription>Stored securely, only shown once.</DialogDescription></DialogHeader>
 <div className="p-4 bg-muted/50 rounded-xl font-mono text-sm border flex items-center justify-between gap-4 mt-4"><span className="truncate flex-1 font-medium text-primary">{createdKey}</span><Button variant="ghost"size="icon"onClick={() => copy(createdKey,"nk")}><Copy className="size-4"/></Button></div>
