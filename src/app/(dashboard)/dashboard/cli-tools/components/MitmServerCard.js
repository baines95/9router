...
export default function MitmServerCard({ apiKeys, cloudEnabled, onStatusChange }) {
 ...
 {isRunning ? (
 <Badge className="h-5 px-1.5 text-xs bg-primary/10 text-primary border-primary/20">Running</Badge>
 ) : (
 <Badge variant="secondary"className="h-5 px-1.5 text-xs">Stopped</Badge>
 )}
 </div>
 ...
 {ok ? <CheckCircle2 className="size-3 text-primary"/> : <XCircle className="size-3 text-muted-foreground"/>}
 <span className={cn(ok ?"text-primary font-medium":"text-muted-foreground")}>{label}</span>
 </div>
 ))}
 </div>
 </div>
 ...
 disabled={loading}
 className="h-8 bg-muted/30 border-border/50 text-muted-foreground hover:bg-muted/30 text-xs"
 >
 ...
 disabled={loading}
 className="h-8 bg-destructive/10 text-destructive border-destructive/20 hover:bg-destructive/20 text-xs"
 >
 ...
 className="h-8 bg-primary/10 text-primary border-primary/20 hover:bg-primary/20 text-xs"
 >
 ...
 {isRunning && (
 <p className="text-xs text-muted-foreground ml-auto">Enable DNS per tool below to activate interception</p>
 )}
 </div>

 {/* Action error */}
 {actionError && (
 <div className="flex items-start gap-2 px-3 py-2 rounded-lg text-xs bg-destructive/10 text-destructive border-destructive/20">
 ...
 {isWindows && !isAdmin && (
 <div className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs bg-destructive/10 text-destructive border-destructive/20">
