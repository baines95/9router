...
export default function DefaultToolCard({ 
 ...
 >
 {copiedField ==="apiKey"? <Check className="size-4 text-primary"/> : <Copy className="size-4"/>}
 </Button>
 </>
 ) : (
 <div className="h-9 flex items-center px-3 bg-muted/20 border border-border/50 rounded-md text-xs text-muted-foreground w-full">
 ...
 className="shrink-0 h-9 w-9"
 >
 {copiedField ==="model"? <Check className="size-4 text-primary"/> : <Copy className="size-4"/>}
 </Button>
 <Button
 variant="ghost"
 size="icon-sm"
...
"bg-primary/10 border-blue-500/30 text-primary dark:text-primary"
 )}
 >
 ...
 <div 
 className="size-8 rounded-full flex items-center justify-center shrink-0 text-sm font-medium text-white"
 ...
 <code className="flex-1 px-3 py-2 bg-muted/50 rounded-lg text-xs font-mono border border-border/50 truncate">
 ...
 >
 {copiedField === `${item.step}-${item.title}` ? <Check className="size-3.5 text-primary"/> : <Copy className="size-3.5"/>}
 </Button>
 )}
 </div>
 )}
 ...
 >
 {copiedField ==="codeblock"? <Check className="mr-1.5 size-3 text-primary"/> : <Copy className="mr-1.5 size-3"/>}
 {copiedField ==="codeblock"?"Đã sao chép!":"Sao chép"}
 </Button>
 </div>
 <pre className="p-4 bg-muted/50 rounded-xl border border-border/50 overflow-x-auto">
 <code className="text-xs font-mono whitespace-pre text-foreground/80 leading-relaxed">
